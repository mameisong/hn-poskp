...
//        prestartAllCoreThreads();
        ...
        if (currentThreadShouldBeStopped()) {
            ...
            if (lastTime + threadRenewalDelay < System.currentTimeMillis()) {
                if (lastTimeThreadKilledItself.compareAndSet(lastTime,
                    ...
                    final String msg = String.format(Constants.STOP_MSG, Thread.currentThread().getName());
    ...
    public void execute(Runnable command) {
        try {
            ...
            logger.debug("poolSize:{}  maxSize:{} queueSize:{} activeSize:{}", getPoolSize(),
                    getMaximumPoolSize(), getQueue().size(), getActiveCount());
        } catch (RejectedExecutionException e) {
            //TODO 处理队列满的情况（外层暂未处理，先捕获）
     ...
     */
    public void shutdown(boolean waitForTasksToCompleteOnShutdown, int awaitTerminationSeconds) {
        if (waitForTasksToCompleteOnShutdown) {
            shutdown();
        } else {
            for (Runnable remainingTask : shutdownNow()) {
                cancelRemainingTask(remainingTask);
            }
        }
        awaitTerminationIfNecessary(awaitTerminationSeconds);
    }

    /**
     * Cancel the given remaining task which never commended execution,
     * as returned from {@link ThreadPoolExecutor#shutdownNow()}.
     *
    ...
    /**
     * Wait for the executor to terminate, according to the value of the {@code awaitTerminationSeconds}
     */
